...
import { formDataSubmit } from '../store/slices/formSubmit';
...
const FormOne = () => {
  ...
  const { data, isLoading, message, isSuccess, isError } = useSelector(
  ...
  const onFormSubmit = async (formData) => {
    ...
    if (key !== "profileImage" && key !== "certificateImage") {
      if (Array.isArray(formData[key])) {
        formData[key].forEach((val, i) => {
          payloadData.append(`${key}[${i}]`, val);
        });
      } else {
  ...
  if (formData.certificateImage && formData.certificateImage[0]) {
    payloadData.append("certificateImage", await fileToBase64(formData.certificateImage[0]));
  }
    ...
    dispatch(formDataSubmit(payload))
      .unwrap()
      .then((response) => {
        Swal.close();
  ...
  // 👇 handle Redux state updates here
  // useEffect(() => {
